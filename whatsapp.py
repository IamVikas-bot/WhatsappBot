...
class WhatsAppElements:
    ...
    attach_icon = (By.XPATH,"//*//span[@data-icon='clip']")
...
class WhatsApp:
    ...
    def send_message(self, name, message):
        ...
        try:
            ...
            raise Exception("ChromeDriver only supports characters in the BMP");
        try:
    ...
    def send_document(self, name, document_location):
        ...
        try:
            ...
            #Send Icon comes after slecting the file
            send_file_xpath = '/html/body/div[1]/div/div/div[2]/div[2]/span/div/span/div/div/div[2]/span/div/div/span'
            #attach_type_xpath = '/html/body/div[1]/div/div/div[4]/div/header/div[3]/div/div[2]/span/div/div/ul/li[3]' \
             #                   '/button/input'
            attach_type_xpath = '//span[@data-testid="attach-document"]'
            # open attach menu
            attach_btn = self.browser.find_element(*WhatsAppElements.attach_icon)
            attach_btn.click()
            ...
            #print("After senketd")
            #ActionChains(self.browser).send_keys_to_element(attach_img_btn,document_location).perform()
            #self.browser.execute_script("document.getElementByXpath('//span[@data-testid='"+'attach-document'+"']').value='"+document_location+"'")
            time.sleep(3)
            send_btn = self.browser.find_element_by_xpath(send_file_xpath)
            send_btn.click()
    ...
    def join_group(self, invite_link):
        ...
        join_chat.click()
        use_WhatsApp_Web = self.browser.find_element_by_xpath("//a[text()='use WhatsApp Web']")
        self.browser.execute_script("arguments[0].click();", use_WhatsApp_Web)

        try:
            WebDriverWait(self.browser, self.timeout).until(EC.presence_of_element_located(
            (By.XPATH, '//div[text()="Join group"]')))
            join_group = self.browser.find_element_by_xpath("//div[text()='Join group']")
            self.browser.execute_script("arguments[0].click();", join_group)
            WebDriverWait(self.browser, self.timeout).until(EC.presence_of_element_located(
            (By.XPATH, "//div[@id='main']//header//span[@dir='auto']")))
            time.sleep(3)
            join_group_name = self.browser.find_element_by_xpath(
                "//div[@id='main']//header//span[@dir='auto']").get_attribute('title')
            newGroups.append(join_group_name)
        except:
            print("You have Already Joined the Group")
        return newGroups
    ...
    def get_last_message_for(self, name):
        ...
        for i in soup.find_all("div", class_="message-in"):
            upperElement = soup.find_all("div", class_="message-in")
            print(len(upperElement))
    ...
    def getGroupsInList(self):
        groups = list()
        file = open("ListOfGroups.txt")
        f = file.readlines()
    ...
    # Get last messages
    def get_unread_message(self, name):
        messages = list()
        element = self.browser.find_elements_by_xpath("//span[contains(@aria-label, 'unread message')]")
        time.sleep(3)
        for ele in element:
            count = ele.text
            ele.click()
            for i in count:
                message2 = self.browser.find_element_by_css_selector("div.message-in span.selectable-text span")
                if message2:
                    messages.append(message2.text)
        messages = list(filter(None, messages))
        return messages

    def get_unread_messages_only(self):
        configs = jproperties.Properties()
        with open('config.properties', 'rb') as config_file:
            configs.load(config_file)
        #getting all the elements with "unread chats"
        unreadChats = self.browser.find_elements_by_xpath(configs.get("unread").data)
        print("Number of unread chats before going into for Loop")
        print(len(unreadChats))

        #iterating the elements
        messages = list()
        for element in unreadChats:
            #clicking on the first element of unread chat
            element.click()
            #Getting unread count of the first element
            unreadCount: int = int(element.text)
            print("Unread Count is :")
            print(unreadCount)
            #Getting name of the Person/Group of chat
            chatName = self.browser.find_element_by_xpath(configs.get("chatName").data).text
            print("chat name is :")
            print(chatName)
            time.sleep(1)
            #getting total number of unread messgaes having unread msgs
            unreadMessages = self.browser.find_elements_by_xpath(configs.get("newUnreadMessages").data)

            print("Length of Unread msgs now :")
            print(len(unreadMessages))

            while len(unreadMessages) < unreadCount:
                element = unreadMessages[len(unreadMessages)-1]
                #self.browser.execute_script("window.scrollBy(0,1000)")
                self.browser.execute_script("arguments[0].scrollIntoView();", element)
                unreadMessages = self.browser.find_elements_by_xpath(configs.get("newUnreadMessages").data)

            print("Length of Unread msgs now after viewport :")
            print(len(unreadMessages))
            ...
            for unreadMessage in unreadMessages:
                ...
                if "Read more" in text:
                    unreadMessage.find_element_by_xpath(configs.get("readMore").data).click()
                    text = unreadMessage.text
                    if text:
    ...
    def read_text_from_Image(self,imagePath,otherPath=r"C:/Users/vv26698/Desktop"):
        pytesseract.pytesseract.tesseract_cmd = otherPath
        return pytesseract.image_to_string(imagePath).replace("\x0e","")
